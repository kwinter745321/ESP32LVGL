...
# Initialize SPI
spi = SPI(1, baudrate=40_000_000, sck=Pin(4), mosi=Pin(5), miso=Pin(0) )
#tspi = SPI(0, baudrate=1_000_000, sck=Pin(18), mosi=Pin(19), miso=Pin(16))
tspi = SPI(1, baudrate=2_000_000, sck=Pin(4), mosi=Pin(5), miso=Pin(0))
...
#disp = ili9xxx.Gc9a01(spi=spi, dc=4, cs=10, rst=5, rot=PORTRAIT)
...
#disp = ili9xxx.Ili9488b(spi=spi, dc=4, cs=10, rst=7, rot=0)
# 3.5 inch ILI9488 Capacitive touch
#disp.write_register(0x21, b"\x00")

disp = ili9xxx.St7796(spi=spi, dc=3, cs=1, rst=2, rot=0)
...
#### touchscreen object ###################################
touch = None
# if disp.display_type in ['ili9488','ili9488b' ,'st7796']:
...
touch = xpt2046.Xpt2046_hw(spi=tspi,cs=11,width=(320),height=(480),rot=disp.rot)
print(tspi)

# i2c = SoftI2C(scl = Pin(9), sda = Pin(8), freq = 400_000, timeout= 1000)
# print(i2c)
# touch = Touch_CST816S(i2c=i2c,int_pin=7,rst_pin=6)  
