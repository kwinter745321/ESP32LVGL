...
class St7796_hw(st77xx.St77xx_hw):
    ...
    def apply_rotation(self, rot):
        self.rot = rot
        #_MADCTL_BGR = 0x04  #certain st7796 needed this hardcoded
        #print("_MADCTL_BGR:", _MADCTL_BGR )
        if (self.rot % 2) == 0:
            self.width, self.height = self.res
        else:
            self.height, self.width = self.res
        try:
            ...
            self.write_register(
                _MADCTL,
                bytes([(temp)|_MADCTL_ROTS[self.rot%4]]))
            #print("_MADCTL_BGR using this:", temp )
        except:
...
class St7796(St7796_hw, st77xx.St77xx_lvgl):
    def __init__(self, doublebuffer=True, factor=8, **kw):
        """See :obj:`Ili9341_hw` for the meaning of the parameters."""
        ######for certain tft, _MADCTL_BGR must = 0x04  see apply _rotation()
