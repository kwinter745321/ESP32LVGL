...
from machine import idle, Pin, SoftSPI  # type: ignore
...
#                            int_handler=self.touchscreen_press)
        self.touch = Touch(spi2, cs=Pin(21), int_pin=Pin(23),
                           int_handler=self.touchscreen_press)
...
def test():
    ...
    spi = SoftSPI(baudrate=80_000_000, sck=Pin(36), mosi=Pin(32), miso=Pin(33) )
    display = Display(spi, dc=Pin(22),  rst=Pin(5), cs=Pin(4),
                      width=240,height=320,rotation=0,mirror=False)
    spi2 = SoftSPI(baudrate=1_000_000, sck=Pin(36), mosi=Pin(32), miso=Pin(33) )
